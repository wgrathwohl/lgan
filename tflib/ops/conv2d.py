...
def Conv2D(name, input_dim, output_dim, filter_size, inputs, he_init=True, mask_type=None, stride=1, weightnorm=None, biases=True, gain=1., lipschitz_constraint=False):
    ...
    with tf.name_scope(name) as scope:
        ...
        if lipschitz_constraint:
            ...
            in_shape = np.prod(inputs.get_shape().as_list()[1:])
            out_shape = np.prod(result.get_shape().as_list()[1:])
            in_norms = tf.norm(tf.reshape(inputs, [-1, in_shape]), axis=1)
            out_norms = tf.norm(tf.reshape(result, [-1, out_shape]), axis=1)
